...
def main(args):
	...
	for epoch in range(start_epoch, args.epochs):
		...
		for i, data in enumerate(loader_train, 0):
			...
			out_train = model(imgn_train, stdn_var) #模型的前向传播
			...
			psnr_train1 += batch_psnr(out_train, img_train, 1.)
		...
		#计算每次epoch的均值
		loss_loop/=len(loader_train)
		loss_train_value.append(loss_loop)

		psnr_train1 /= len(loader_train)
		psnr_train_value.append(psnr_train1)
		...
		for valimg in dataset_val:
			...
			loss_val = criterion(out_val, sigma_noise) / (imgn_val.size()[0] * 2)
			loss_val_temp+=loss_val.data
		# 计算每次epoch的均值
		loss_val_temp/=len(dataset_val)
		loss_val_value.append(loss_val_temp)

		psnr_val /= len(dataset_val)
		psnr_val_value.append(psnr_val)
